...
export default function CampaignHistoryModal({ open, onOpenChange }: CampaignHistoryModalProps) {
    ...
    useEffect(() => {
        ...
        const fetchCampaigns = async () => {
            setLoading(true);
            try {
                const res = await fetch(`${API_BASE_URL}/campaigns/history`, { credentials: "include" });
    ...
    const getStatusBadge = (status: string) => {
        ...
        return <Badge className={cn("px-3 py-1 rounded-full border", styles[status] || "bg-zinc-50")}>{status}</Badge>;
    };
        ...
        <div className="flex flex-col h-full bg-white">
            <div className="p-6 border-b shrink-0">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        <h2 className="text-2xl font-bold text-zinc-900">Campaign History</h2>
                    </div>
                </div>
                <div className="flex flex-col sm:flex-row gap-4">
                    <div className="relative flex-1">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
                        <Input placeholder="Search campaigns..." className="pl-10 rounded-xl" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                    </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                {loading ? (
                    ...
                    <Table>
                        <TableHeader><TableRow><TableHead>Campaign</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Data</TableHead></TableRow></TableHeader>
                        <TableBody>
        ...
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-8">
                <div>
                    ...
                    <p className="text-lg text-zinc-500 mt-1 font-medium">Brand: {selectedCampaign!.brand}</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-6 rounded-3xl border-2 border-zinc-50 bg-zinc-50/50 text-center">
                        <p className="text-xs font-black text-zinc-400 uppercase mb-1">Reach</p>
                        <p className="text-3xl font-black text-zinc-900">{selectedCampaign!.reach}</p>
                    </div>
                    <div className="p-6 rounded-3xl border-2 border-zinc-50 bg-zinc-50/50 text-center">
                        <p className="text-xs font-black text-zinc-400 uppercase mb-1">Engagement</p>
                    ...
                    <div className="p-6 rounded-3xl border-2 text-center" style={{ borderColor: BRAND.light, backgroundColor: `${BRAND.light}11` }}>
                        <p className="text-xs font-black uppercase mb-1" style={{ color: BRAND.primary }}>Earnings</p>
                        <p className="text-3xl font-black" style={{ color: BRAND.primary }}>${selectedCampaign!.earnings.toLocaleString()}</p>
                    </div>
                </div>
                <div className="space-y-3">
                    <h4 className="font-bold text-zinc-900 uppercase text-xs tracking-widest">Description</h4>
            ...
            <div className="p-6 border-t shrink-0 flex justify-between gap-4">
                <Button variant="outline" onClick={() => setViewMode("list")} className="rounded-full px-8 font-bold">Back</Button>
                <Button onClick={() => setViewMode("report")} className="rounded-full px-8 text-white font-bold" style={{ backgroundColor: BRAND.primary }}>View Analytics</Button>
            </div>
        </div>
    );
    ...
    const renderReportView = () => (
        <div className="flex flex-col h-full bg-white">
            <div className="p-4 border-b shrink-0 flex justify-between items-center">
                ...
                <div className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-300">Certified Metrics</div>
            </div>
            <div className="flex-1 overflow-y-auto p-12 space-y-12">
                <div className="text-center">
                    <h3 className="text-4xl font-black text-zinc-900 mb-2">{selectedCampaign!.name}</h3>
                    <p className="text-zinc-400 font-bold uppercase tracking-widest text-xs">Performance Since - {formatDate(selectedCampaign!.date)}</p>
                </div>
                <div className="grid grid-cols-2 gap-12 max-w-2xl mx-auto">
                    <div className="text-center border-r">
                        <p className="text-xs font-black text-blue-500 uppercase mb-2">Impressions</p>
                        <p className="text-6xl font-black tracking-tighter text-zinc-900">{selectedCampaign!.reach}</p>
                    </div>
                    <div className="text-center">
                        <p className="text-xs font-black text-zinc-400 uppercase mb-2">Engagement Rate</p>
                ...
                <div className="space-y-6 max-w-lg mx-auto p-8 rounded-[40px] bg-zinc-50 border border-zinc-100">
                    <p className="text-xs font-black text-zinc-400 uppercase text-center tracking-widest mb-4">Traffic Insights</p>
            ...
            <div className="p-8 border-t shrink-0 flex justify-between items-center">
                ...
                >
                    {isExporting ? <div className="h-4 w-4 animate-spin border-2 border-white/30 border-t-white rounded-full" /> : <><Download className="mr-2 h-4 w-4" /> Download PDF</>}
                </Button>
            </div>
        </div>
    );
