...
export const AddPostDialog = ({ isOpen, onClose, onSubmit }: AddPostDialogProps) => {
    ...
    >
      {isLoadingImage ? (
        <div className="flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-muted-foreground text-sm">{t("addPost.processImage")}</p>
        </div>
        ...
        <>
          ...
          {!isMobileView && <p className="text-sm text-muted-foreground">PNG, JPG {t("addPost.upTo")} 10MB</p>}
        </>
      )}
      {!newPost.imagePreview && (
        <input
  ...
  const renderMobileStep1 = () => (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-center">{t("addPost.uploadImage")}</h2>
  ...
  const renderMobileStep2 = () => (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-center">{t("addPost.campaignTitle")}</h2>
      <Input 
        value={newPost.title} 
      ...
      <div className="flex gap-2 mt-4">
        <Button variant="outline" className="flex-1" onClick={() => setMobileStep(1)}>{t("addPost.back")}</Button>
        <Button className="flex-1" disabled={!newPost.title} onClick={() => setMobileStep(3)}>{t("addPost.next")}</Button>
      </div>
    </div>
  );
  ...
  const renderMobileStep3 = () => (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-center">{t("addPost.content")}</h2>
      <Input 
        value={newPost.brand} 
      ...
      >
        <option value="">{t("addPost.selectType")}</option>
        <option value="Instagram Reel">Instagram {t("addPost.reel")}</option>
        <option value="TikTok Video">TikTok {t("addPost.video")}</option>
        <option value="YouTube Video">YouTube {t("addPost.video")}</option>
        <option value="Instagram Post">Instagram {t("addPost.post")}</option>
        <option value="Blog Article">{t("addPost.blogArticle")}</option>
        <option value="UGC">UGC</option>
      </select>
      <div className="flex gap-2 mt-4">
        <Button variant="outline" className="flex-1" onClick={() => setMobileStep(2)}>{t("addPost.back")}</Button>
        <Button className="flex-1" onClick={() => setMobileStep(4)}>{t("addPost.next")}</Button>
      </div>
    </div>
  );
  ...
  const renderMobileStep4 = () => (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-center">{t("addPost.desc")}</h2>
      <Textarea 
        rows={5} 
      ...
      />
      <div className="flex gap-2 mt-4">
        <Button variant="outline" className="flex-1" onClick={() => setMobileStep(3)}>{t("addPost.back")}</Button>
        <Button className="flex-1" onClick={handleSubmit} disabled={isSubmitting}>
          {isSubmitting ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />{t("addPost.publishing")}</> : t("addPost.publish")}
        </Button>
      </div>
    </div>
  );
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="overflow-hidden max-w-[95vw] sm:max-w-[700px] max-h-[90vh] flex flex-col p-[20px]">
        <div className="px-5 py-4 sm:px-7 sm:py-5 border-b flex-shrink-0">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl">{t("addPost.addNewWork")}</DialogTitle>
        ...
        {isMobile ? (
          ...
          <div className="flex-grow overflow-y-auto p-6 space-y-6">
            <div className="grid grid-cols-2 gap-6">
              <div>
                <Label className="text-sm font-medium mb-2 block">{t("addPost.campaignImage")} *</Label>
                {renderImageUpload()}
              </div>

              <div className="space-y-4">
                <div>
                  <Label htmlFor="title" className="text-sm font-medium">{t("addPost.campTitle")} *</Label>
                  <Input
                    id="title"
                    value={newPost.title}
                    onChange={(e) => setNewPost({ ...newPost, title: e.target.value })}
                ...
                <div>
                  <Label htmlFor="brand" className="text-sm font-medium">{t("addPost.brandName")}</Label>
                  <Input
                    id="brand"
                    value={newPost.brand}
                    onChange={(e) => setNewPost({ ...newPost, brand: e.target.value })}
                ...
                <div>
                  <Label htmlFor="type" className="text-sm font-medium">{t("addPost.contentType")}</Label>
                  <select
                    id="type"
                ...
                <div>
                  <Label htmlFor="description" className="text-sm font-medium">{t("addPost.desc")}</Label>
                  <Textarea
                    id="description"
                    rows={4}
                    value={newPost.description}
        ...
        <div className="px-5 py-3 sm:px-7 sm:py-4 bg-muted/30 border-t flex justify-end gap-3 flex-shrink-0">
          <Button variant="outline" onClick={handleClose}>{t("addPost.cancel")}</Button>
          {!isMobile && (
            <Button 
